...
from app.utils.formatacao import apenas_digitos, formatar_cpf, formatar_masp
...
def _validar_cpf(cpf: str) -> str:
    ...
    def _digito_verificador(parcial: str) -> str:
        ...
        soma = sum(int(digito) * peso for digito, peso in zip(parcial, range(peso_inicial, 1, -1)))
        resto = (soma * 10) % 11
...
class CidadaoPublicoResponse(BaseModel):
    """Resposta mínima do totem para confirmação de identidade."""
    id: int
    nome: str
    model_config = ConfigDict(from_attributes=True)
...
class CidadaoIdentificar(BaseModel):
    ...
    def validar_termo(cls, valor: str) -> str:
        valor = valor.strip()
        if len(valor) < 3:
            raise ValueError("Informe pelo menos 3 caracteres do CPF, MASP ou nome.")
        return valor
